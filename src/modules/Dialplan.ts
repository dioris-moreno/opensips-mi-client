...
export class Dialplan extends Module {
     ...
     * @param [params.partition] - Partition to be reloaded. If not specified, all partitions will be reloaded.
     */
     ...
     * @param [params.partition] - the name of the partition when the dpid is located
     */
    translate = (params: { dpid: number; input: string; partition?: string }) => this.execute('dp_translate', params);

    /**
     * Display partition(s) details.
     * @param [params.partition] - The partition name. If no partition is specified, all known partitions will be listed.
     */
